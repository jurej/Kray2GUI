...
//custom functions
	...
	rendersequence{
		if (v_output_On && size(v_outname)>0 || v_render0){        // prevent rendering if no output is specified
             if (exe_path!=""){
            ...
            warn("There is no output selected!");
        }
	}
...
//Save krayscript file temp scene
	save_krayscript_file:krayfile,temp_scene{
		...
		// auto gir size
			...
			//if (v_limitdr==3){													Disabled because it causet images saved to disk to loose gamma setting.
