...
maxlinelength=30; //if line is longer lscript will crash
...
options
{
	...
	ui_window_h			= 480 + modal * 40;					// Window height
	...
	c_custom_command = ctlstring("Custom Cmd",custom_command);
	ctlposition(c_custom_command, gad_x, ui_offset_y, ui_window_w - 16, gad_h, gad_text_offset);
	...
	c_volumetric,c_skyON,c_sunON,c_SunIntensity,c_SunShadow,c_SunPower,c_custom_command);
		...
		if (!create_flag){	// not inited, do not save settings
		    ...
		    if (mlsize==0){
				...
				for (a=1 ; a<=mlsize ; a+=maxlinelength){
					...
					t=-a+t;
					io.writeln(int,601);
					io.writeln(strsub(custom_command,a,t));
				}
			}
		}
...
get_values {
	...
	custom_command=getvalue(c_custom_command);
}
	...
	if (loaded_version>0){
		...
		while(hunk>=100){
			...
			switch(hunk){
				...
				break;
				case 601:
					load_custom_command=load_custom_command+read_line(io);
				break;
				default:
			...
			custom_command=load_custom_command;
		}
...
save_krayscript:krayfile{
	if (v_onFlag){
		...
		krayfile.writeln(custom_command+";");
